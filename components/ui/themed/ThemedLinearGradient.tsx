...
export const ThemedLinearGradient: React.FC<Props> = ({
  ...
  amountTop = 0.05,
  amountBottom = 0.1,
  start = [0, 0] as LinearGradientPoint,
  end = [1, 1] as LinearGradientPoint,
  fullscreen = true,
  pointerEvents = "none",
  ...rest
}) => {
  ...
  const bottom = toColor
    ? rgba(toColor, alpha)
    : rgba(
        isLight ? darken(amountBottom, base) : lighten(amountBottom, base),
        alpha,
      );

  const colors = useMemo(() => [top, bottom], [top, bottom]);
