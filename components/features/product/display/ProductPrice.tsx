...
export const ProductPriceSimple = React.memo(function ProductPriceSimple({
    productPrices,
    productAvailability,
    showIcon = false,
    ...textProps
    ...
    const label = isFree ? 'Gratis!' : formatPrice(productPrices, { field: 'price' });
    return (
        <PriceLine showIcon={showIcon && (isFree || isOnSale)}>
            <ThemedText {...textProps} subtle={subtle}>
                {label}
...
export const ProductPriceRange = React.memo(function ProductPriceRangeCmp({
    ...
    const label = minVal === maxVal
        ...
        : short ? `Fra ${formatPrice(min, { field: 'price' })}` : 
