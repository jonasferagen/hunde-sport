...
export function PurchaseFlow({ product }: Props) {
  ...
  const purchasable = React.useMemo(() => Purchasable.create({ product }), [product]);

  return <PurchaseButton purchasable={purchasable} onOpenCustomization={onOpenCustomization} onOpenVariations={onOpenVariations} />;
}
...
const onOpenVariations = async (p: Purchasable) => {
  const { ProductVariationsModal } = await import("@/components/features/product-variation/ProductVariationsModal");
  openModal((_, api) => <ProductVariationsModal purchasable={p} close={() => api.close()} />);
};
