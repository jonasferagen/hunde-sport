...
type Props = {
    ...
    onRequestVariation?: (p: Purchasable) => void;
};
...
export const PurchaseButtonSmart = React.memo(function PurchaseButtonSmart({
    ...
    const onPress = React.useCallback(async () => {
        ...
        if (cta.mode === 'select-variation') {
            ...
            onRequestVariation?.(purchasable);    // delegate opening the modal
            return;
        }
    ...
    const disabled = loading || (inModal ? !purchasable.isValid : cta.mode === 'unavailable');
...
export type PurchaseCTAState = { mode: PurchaseCTAMode; label: string; disabled: boolean };

export function computeCTA(p: Purchasable, ctx: 'card' | 'modal'): PurchaseCTAState {
