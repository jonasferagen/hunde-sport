...
import { ThemedInput, ThemedXStack, ThemedYStack } from "@/components/ui/themed";
...
function CustomFieldRow({ field, onChange }: { field: CustomField; onChange: (updated: CustomField) => void }) {
  ...
  return (
    <ThemedYStack key={field.key}>
      <ThemedXStack split mb="$1.5">
        ...
        <ThemedInput id={field.key} value={field.value ?? ""} onChangeText={handleChange} maxLength={field.maxlen} multiline={false} />
      </ThemedXStack>
    </ThemedYStack>
  );
}
...
export const ProductCustomizationForm: React.FC<Props> = ({ fields, onChange }) => {
