...
export function makeTemplate(base: string): ThemeTemplate {
  ...
  const borderBase = darkenBorders
    ? darken(AMOUNTS.borderBase, base)
    : lighten(AMOUNTS.borderBase, base);
  const borderStep = (amt: number) =>
    darkenBorders ? darken(amt, borderBase) : lighten(amt, borderBase);
  ...
  return {
    ...
    shadowColorHover: rgba(
      "#000",
      Math.min(1, shadowAlpha + AMOUNTS.shadowHoverDelta)
    ),
    shadowColorPress: rgba(
      "#000",
      Math.min(1, shadowAlpha + AMOUNTS.shadowPressDelta)
    ),
    shadowColorFocus: rgba(
      "#000",
      Math.min(1, shadowAlpha + AMOUNTS.shadowFocusDelta)
    ),
  };
}
