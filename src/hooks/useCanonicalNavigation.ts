...
type ArgsOf<K extends RouteKey> = Routes[K] extends {
  path: (...a: infer A) => any;
}
  ? A
  : never;
...
export function useCanonicalNavigation() {
  ...
  const pathname = React.useMemo(
    () => stripTrailingSlash(rawPathname),
    [rawPathname],
  );

  const paramsObj = useLocalSearchParams();
  const currentParamsKey = React.useMemo(
    () => paramsKey(paramsObj as any),
    [paramsObj],
  );
  ...
  const coreTo = React.useCallback(
    async <K extends RouteKey>(
      key: K,
  ...
  const to = React.useCallback(
    <K extends RouteKey>(key: K, ...args: ArgsOf<K>) => {
      void coreTo(key, false, ...args);
    },
    [coreTo],
  );

  const toWithoutOverlay = React.useCallback(
    <K extends RouteKey>(key: K, ...args: ArgsOf<K>) => {
  ...
  const href = React.useCallback(
    <K extends RouteKey>(key: K, ...args: ArgsOf<K>): HrefObject =>
...
async function ensureTransientUiClosed() {
  ...
  await Promise.all([
    wantsModalClose
      ? useModalStore.getState().waitUntilClosed()
      : Promise.resolve(),
    wantsDrawerClose
      ? useDrawerStore.getState().waitUntilClosed()
      : Promise.resolve(),
  ]);
}
